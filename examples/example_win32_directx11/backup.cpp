...
bool parkCarActionSpecific = false; // Sets the state of the child window
...
int main(int, char**)
{
    ...
    while (!done)
    {
        ...
        if (ImGui::Begin("Sistema Estacionamiento UP", &opened, ImGuiWindowFlags_NoResize || ImGuiWindowFlags_NoMove || ImGuiWindowFlags_NoCollapse)) { // Starts the first window
            ...
            if (ImGui::Button("Estacionar Carro Optimizado", ImVec2(300, 50))) { // Button to open Park Car menu
                takeCarAction = false; // Close the other menu if open
                parkCarAction = true; // Opens the menu
            }
            ImGui::SameLine();
            if (ImGui::Button("Estacionar Carro Especificamente", ImVec2(300, 50))) { // Button to open Park Car Specific menu
                takeCarAction = false; // Close the other menu if open
                parkCarActionSpecific = true; // Opens the menu
            }
            ImGui::SameLine();
            ...
            if (parkCarActionSpecific == true) {
                if (ImGui::BeginChild("Estacionar Especificamente", ImVec2(1280, 600), true)) {
